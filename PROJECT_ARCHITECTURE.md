...
4. **Validate**: compare your recovered treatment effect against **`true_realised_att_pct`** in the ground truth file and report the recovery error explicitly, e.g. *"DiD estimate: +0.675 log points vs a target of +0.5935 (+0.082 error, 95% CI [0.666, 0.684] does not cover truth)."* This comparison is what confirms the method is recovering something real, rather than producing a plausible-looking number that happens to fit the noise.

   **Not `true_promo_uplift_pct`.** That column is a structural coefficient applied per 10 percentage points of discount (`promo_effect = 1 + uplift * discount/10`), and it compounds with the separate price-elasticity response. It is an input to the simulation, not an average of what the simulation did, so scoring an estimator against it compares two different quantities and the resulting "recovery error" means nothing. `true_realised_att_pct` is the effect actually applied to latent demand, accumulated over each SKU's treated rows, and is the only column in that file that a treatment-effect estimate is comparable with. It is NaN for the 24 never-promoted SKUs.

   Three further adjustments stand between the two numbers, and `src/causal/estimands.py` is what constructs the target: a log-scale estimate is not the arithmetic percentage (a Jensen gap of 12.4pp here), and estimates recovered from `units_sold` sit below the latent ATT because sales are stockout-censored. `reports/phase4_causal.md` carries the figures.
